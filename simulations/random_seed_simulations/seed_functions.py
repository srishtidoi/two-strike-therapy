...
def strike(N, N_tau, threshold_type, i, t, t_min):
    """changing the treatment to strike 2 after a certain threshold. 
    The threshold type tells if the strike is to be applied before or 
    after the population minima """

    if i==0:
        if threshold_type=='before-min':
            if N<=N_tau: i=1
    
        if threshold_type=='after-min':
            if t>=t_min and N>=N_tau: i=1
...
def first_run():
    print("first run: without second treatment; seed=", seed)
    rnd.seed(seed)
    ...
    pop_history = []
    time_history = []
    pop_history.append(np.insert(pop_dist, 0, [0,0]))
    time_history.append([0,0,0])
    ...
    while stop_flag==0:
        ...
        if (t-t_record)>=t_res:
            t_record = t
            pop_history.append(np.insert(pop_dist_new, 0, [0,0]))
            time_history.append([0, 0, t_record])
        ...
        if t>100 & N>=min(0.99*K, N_max):
            stop_flag=1
            outcome = 1 # progression
        if t>=T:
            stop_flag=1
            if N>=N_init: outcome = 1 # progression
            else: outcome = 2 # persistence

    return [pop_history, time_history, outcome, N_min, t_min]

def one_run(run, N_tau, threshold_type, t_min):
    print("N_tau= ", N_tau, "; ", threshold_type)
    rnd.seed(seed)
    ...
    pop_history = []
    time_history = []
    pop_history.append(np.insert(pop_dist, 0, [run,N_tau]))
    time_history.append([run,N_tau,0])
    ...
    while stop_flag==0:
        ...
        i = fn.strike(N, N_tau, threshold_type, i, t, t_min)
        birth_rates = (lambdas_vector - ((lambdas_vector-mus_vector)*N/K))*pop_dist 
        death_rates = (mus_vector+fn.therapy(i))*pop_dist
        mutation_rates = (mutation_vector[i]*np.array([alpha_1*pop_dist[0], alpha_2*pop_dist[0], alpha_2*pop_dist[1], alpha_1*pop_dist[2]]))
        ...
        if (t-t_record)>=t_res:
            t_record = t
            pop_history.append(np.insert(pop_dist_new, 0, [run,N_tau]))
            time_history.append([run, N_tau, t_record])
        ...
        if N==0: 
            stop_flag=1
            outcome = 0 # extinction
        if t>100 and N>=min(0.99*K, N_max):
            stop_flag=1
            outcome = 1 # progression
        if t>=T:
            stop_flag=1
